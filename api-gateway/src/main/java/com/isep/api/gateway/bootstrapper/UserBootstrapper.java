package com.isep.api.gateway.bootstrapper;
...
public class UserBootstrapper implements CommandLineRunner, Ordered {
    ...
    public void run(String... args) throws Exception {
        ...
        if (userRepo.findByUsername("admin1@mail.com").isEmpty()) {
            User admin1 = new User("admin1@mail.com", encoder.encode("AdminPW1"), "Jose Antonio", "355489123",
                    "Rua " + "Um");
        ...
        if (userRepo.findByUsername("admin2@mail.com").isEmpty()) {
            User mod1 = new User("admin2@mail.com", encoder.encode("AdminPW2"), "Antonio Jose", "321984553",
                    "Rua " + "dois");
        ...
        if (userRepo.findByUsername("user1@mail.com").isEmpty()) {
            ...
            user1.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user1);
        }
        if (userRepo.findByUsername("user2@mail.com").isEmpty()) {
            User user2 = new User("user2@mail.com", encoder.encode("userPW2"), "Miguel Nuno", "253698854", "Rua " +
                    "quatro");
            user2.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user2);
        }
        if (userRepo.findByUsername("user3@mail.com").isEmpty()) {
            User user3 = new User("user3@mail.com", encoder.encode("userPW3"), "Antonio Pedro", "254148863",
                    "Rua " + "vinte");
            user3.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user3);
        }

        if (userRepo.findByUsername("user4@mail.com").isEmpty()) {
            User user4 = new User("user4@mail.com", encoder.encode("userPW4"), "Pedro Antonio", "452369871",
                    "Rua " + "cinco");
            user4.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user4);
        }
        if (userRepo.findByUsername("user5@mail.com").isEmpty()) {
            ...
            user5.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user5);
        }
        if (userRepo.findByUsername("user6@mail.com").isEmpty()) {
            ...
            user6.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user6);
        }
        if (userRepo.findByUsername("user7@mail.com").isEmpty()) {
            ...
            user7.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user7);
        }
        if (userRepo.findByUsername("user8@mail.com").isEmpty()) {
            ...
            user8.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user8);
        }
        if (userRepo.findByUsername("user9@mail.com").isEmpty()) {
            ...
            user9.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user9);
        }
        if (userRepo.findByUsername("user10@mail.com").isEmpty()) {
            User user10 = new User("user10@mail.com", encoder.encode("userPW10"), "Antonio Marco", "201023056", "Rua "
                    + "onze");
            user10.addAuthority(new Role(Role.REGISTERED_USER));
            userRepo.save(user10);
        }
        if (userRepo.findByUsername("user11@mail.com").isEmpty()) {
            User user11 = new User("user11@mail.com", encoder.encode("userPW11"), "Rui Ricardo", "748526326",
                    "Rua " + "doze");
            user11.addAuthority(new Role(Role.REGISTERED_USER));
