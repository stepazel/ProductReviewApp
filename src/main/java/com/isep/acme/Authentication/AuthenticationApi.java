    ...
    public ResponseEntity<UserView> login(@RequestBody @Valid final AuthenticationRequest request) {
        try {
            ...
            final JwtClaimsSet claims =
                    JwtClaimsSet.builder().issuer("example.io").issuedAt(now).expiresAt(now.plusSeconds(expiry)).subject(format("%s,%s", user.getUserId(), user.getUsername())).claim("roles", scope).build();
